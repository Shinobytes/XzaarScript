...
{
    internal class XzaarScriptDiscoveryVisitor : IXzaarDiscoveryVisitor
    {
        private XzaarScriptCompilerContext ctx;

        public XzaarScriptDiscoveryVisitor(XzaarScriptCompilerContext ctx)
        {
            this.ctx = ctx;
        }
        ...
        public object Visit(XzaarExpression expression)
        {
            ...
            if (ctx.StackRecursionCount > 10)
                throw new XzaarCompilerException("PANIC!! Stackoverflow! " + expression.GetType() + " [" + expression.NodeType + "] not implemented");
