...
{
    public class XzaarScriptCompiler : IScriptCompilerVisitor
    {
        private readonly List<string> errors = new List<string>();
        private readonly XzaarScriptCompilerContext ctx;
        private XzaarExpressionType lastBinaryOperationType;
        private bool insideBinaryOperation;
        private VariableReference lastMemberAccessChainResult;

        internal XzaarScriptCompiler(XzaarScriptCompilerContext ctx)
        {
            this.ctx = ctx;
        }
        ...
        public static XzaarAssembly Compile(XzaarAnalyzedTree analyzedTree)
        {
            List<string> errors;
            return Compile(analyzedTree, out errors);
        }

        public static XzaarAssembly Compile(XzaarAnalyzedTree analyzedTree, out List<string> errors)
        {
            var compilerContext = new XzaarScriptCompilerContext(Guid.NewGuid().ToString(), analyzedTree);
            var compiler = new XzaarScriptCompiler(compilerContext);
            var discovery = new XzaarScriptDiscoveryVisitor(compilerContext);
        ...
        public object Visit(MemberAccessChainExpression access)
        {
            ...
            {
                // a.b.c
                if (insideBinaryOperation && lastBinaryOperationType == XzaarExpressionType.Assign && !(targetField is FieldReference))
                {
                    // make sure we return
                    //   StructGet
                    //   StructSet
            ...
            if (instance != null)
            {
                ...
                if (insideBinaryOperation && lastBinaryOperationType == XzaarExpressionType.Assign)
                {
        ...
        public object Visit(VariableDefinitionExpression definedVariable)
        {
            ...
            {
                if (definedVariable.AssignmentExpression.NodeType == XzaarExpressionType.CreateStruct)
                {
