...
{
    internal class ES6ExpressionVisitor : IStringExpressionVisitor
    {
        ...
        public ES6ExpressionVisitor(QuoutePreference quoutePreference)
        {
        ...
        public string Visit(FieldExpression node)
        {
            ...
            codeWriter.Write(node.Name, currentIndent);
            codeWriter.NewLine();
            return codeWriter.ToString();
        }
        ...
        {
            var codeWriter = new XzaarCodeWriter();
            codeWriter.Write("while (true) {", currentIndent);
            codeWriter.NewLine();
            if (loop.Body != null)
            {
                currentIndent++;
        ...
        {
            var codeWriter = new XzaarCodeWriter();
            codeWriter.Write("let " + definedVariable.Name, IsInsideExpression ? 0 : currentIndent);
        ...
        public string Visit(ConstantExpression value)
        {
            ...
            else if (value.Type.Equals(XzaarBaseTypes.Boolean))
            {
                codeWriter.Write(value.Value.ToString().ToLower());
            }
            ...
            {
                var quouteChar = quoutePreference == QuoutePreference.DoubleQuoute ? "\"" : "'";
                codeWriter.Write(quouteChar + value.Value + quouteChar + arrayValue);
            }
    ...
    public static class StringExtensions
    {
        ...
        /// <returns></returns>
        public static string TrimWsRn(this string str)
        {
